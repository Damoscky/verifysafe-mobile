...
import 'package:verifysafe/core/data/view_models/search_worker_view_model.dart';
...
class SearchWorkers extends ConsumerStatefulWidget {
  const SearchWorkers({super.key});

  @override
  ConsumerState<SearchWorkers> createState() => _SearchWorkersState();
}

class _SearchWorkersState extends ConsumerState<SearchWorkers> {
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) {
      _scrollListener();
      ref.read(searchWorkerViewModel).workers.clear();
  ...
  _scrollListener() {
    final vm = ref.watch(searchWorkerViewModel);
      ...
      if (_scrollController.position.pixels ==
        ...
        if (vm.paginatedState != ViewState.error) {
          ...
          if (vm.paginatedState != ViewState.busy &&
            ...
            //fetch more
            vm.searchWorker(firstCall: false, query: _search.text);
          }
        }
      }
    });
  ...
  Widget build(BuildContext context) {
    final vm = ref.watch(searchWorkerViewModel);
    return Scaffold(
      appBar: customAppBar(context: context, title: 'Search for Worker'),
      body: Column(
        children: [
          Padding(
            ...
            child: CustomTextField(
              ...
              onChanged: (value) {
                _debouncer.performAction(
                  action: () async {
                    await vm.searchWorker(query: value);
                  },
                );
              },
            ...
            child: ListView(
              ...
              children: [
                ...
                if (vm.secondState == ViewState.busy)
                  ...
                  WorkersData(workers: vm.workers, emptyStateTitle: ''),
                if (vm.paginatedState == ViewState.busy)
                  Column(
                    children: [
                      SizedBox(height: 8.h),
