import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:verifysafe/core/constants/app_dimension.dart';
import 'package:verifysafe/core/constants/app_theme/custom_color_scheme.dart';
import 'package:verifysafe/core/constants/color_path.dart';
import 'package:verifysafe/core/constants/named_routes.dart';
import 'package:verifysafe/core/data/models/user.dart';
import 'package:verifysafe/core/utilities/navigator.dart';
import 'package:verifysafe/ui/pages/profile/edit_employment_details.dart';
import 'package:verifysafe/ui/widgets/clickable.dart';
import 'package:verifysafe/ui/widgets/custom_appbar.dart';
import 'package:verifysafe/ui/widgets/data_tile.dart';
import 'package:verifysafe/ui/widgets/document_widget.dart';

class ViewEmploymentDetails extends StatelessWidget {
  final bool canEdit;
  final User data;
  const ViewEmploymentDetails({
    super.key,
    this.canEdit = true,
    required this.data,
  });

  @override
  Widget build(BuildContext context) {
    final textTheme = Theme.of(context).textTheme;
    final colorScheme = Theme.of(context).colorScheme;
    return Scaffold(
      appBar: customAppBar(
        context: context,
        title: 'Employment Details',
        actions: [
          if (canEdit)
            Padding(
              padding: EdgeInsets.only(right: 24.w),
              child: Clickable(
                onPressed: () {
                  pushNavigation(
                    context: context,
                    widget: EditEmploymentDetails(workerInfo: data.workerInfo ?? WorkerInfo(),),
                    routeName: NamedRoutes.employmentDetails,
                  );
                },
                child: Text(
                  "Edit",
                  style: textTheme.bodyMedium?.copyWith(
                    fontWeight: FontWeight.w700,
                    color: ColorPath.meadowGreen,
                  ),
                ),
              ),
            ),
        ],
        showBottom: true,
      ),
      body: ListView(
        padding: EdgeInsets.symmetric(
          horizontal: AppDimension.paddingLeft,
          vertical: 16.h,
        ),
        children: [
          Text(
            "Employment Details",
            style: textTheme.bodyMedium?.copyWith(color: colorScheme.text4),
          ),
          SizedBox(height: 16.h),

          DataTile(
            title: "Job Category",
            data: data.workerInfo?.jobCategory ?? "",
          ),
          SizedBox(height: 16.h),
          DataTile(
            title: "Specific Role",
            data: data.workerInfo?.jobRole ?? "",
          ),
          SizedBox(height: 16.h),
          DataTile(
            title: "Years of Experience",
            data: "${data.workerInfo?.experience ?? ""} years",
          ),
          SizedBox(height: 16.h),
          DataTile(
            title: "Language Spoken",
            data: data.workerInfo?.language ?? "",
          ),
          SizedBox(height: 16.h),
          DataTile(
            title: "Willingness to Relocate",
            data: data.workerInfo?.relocatable == 1  || data.workerInfo?.relocatable == true ? "Yes" : "No",
          ),
          SizedBox(height: 32.h),
          if (data.workerInfo?.resumeUrl != null &&
              (data.workerInfo?.resumeUrl?.isNotEmpty ?? false))
            DocumentWidget(
              fileName: data.workerInfo?.resumeUrl,
              onPressed: () {},
            ),

          // //worker emplyment info
          // if (1 + 1 == 3) //todo: worker employment details
          //   Column(
          //     crossAxisAlignment: CrossAxisAlignment.start,
          //     children: [
          //       DataTile(title: "Job Category", data: "NIN"),
          //       CustomDivider(verticalSpace: 14),
          //       DataTile(title: "Specific Role", data: "Driver"),
          //       CustomDivider(verticalSpace: 14),
          //       DataTile(title: "Years of Experience", data: "5 years"),
          //       CustomDivider(verticalSpace: 14),
          //       Text(
          //         "Language Spoken",
          //         style: textTheme.bodyMedium?.copyWith(
          //           color: colorScheme.text5,
          //         ),
          //       ),
          //       SizedBox(height: 12.h),
          //       Wrap(
          //         runSpacing: 6.w,
          //         spacing: 6.w,
          //         children: List.generate(
          //           2,
          //           (index) => Container(
          //             padding: EdgeInsets.all(8.w),
          //             decoration: BoxDecoration(
          //               borderRadius: BorderRadius.circular(8.r),
          //               border: Border.all(color: ColorPath.gullGrey),
          //             ),
          //             child: Text("English"),
          //           ),
          //         ),
          //       ),
          //       CustomDivider(verticalSpace: 14),
          //       Text(
          //         "Willingness to Relocate",
          //         style: textTheme.bodyMedium?.copyWith(
          //           color: colorScheme.text5,
          //         ),
          //       ),
          //       SizedBox(height: 12.h),
          //       Row(
          //         children: [
          //           Row(
          //             children: [
          //               CustomRadioButton(
          //                 borderColor: colorScheme.brandColor,
          //                 value: true,
          //                 onchanged: (value) {},
          //               ),
          //               SizedBox(width: 12..w),
          //               Text("Yes"),
          //             ],
          //           ),
          //           SizedBox(width: 16.w),
          //           SizedBox(width: 12..w),
          //           Row(
          //             children: [
          //               CustomRadioButton(value: false),
          //               SizedBox(width: 12..w),
          //               Text("No"),
          //             ],
          //           ),
          //         ],
          //       ),
          //       CustomDivider(verticalSpace: 14),
          //       DocumentWidget(),
          //     ],
          //   ),
        ],
      ),
    );
  }
}
